...
frappe.pages["engineering-dashboard"].on_page_load = function (wrapper) {
	...
	if (frappe.user.has_role("System Manager")) {
		page.add_menu_item(__("Load Demo Data"), () => load_demo(page));
		page.add_menu_item(__("Create Customer Test User"), () => create_demo_user());
		page.add_menu_item(__("Delete Demo Data"), () => delete_demo(page));
	}
	load(page);
};
...
function load(page) {
	page.__load_demo = () => load_demo(page);
	page.__create_demo_user = () => create_demo_user();
...
function render(page, data) {
	...
	$(page.body).html(`
		<style>
			...
			.edb-empty { border: 1px dashed var(--border-color); border-radius: 10px;
				padding: 16px; margin-bottom: 12px; background: var(--subtle-fg, #f8fafc);
				display: flex; flex-direction: column; gap: 6px; align-items: flex-start; }
			.edb-empty-title { font-weight: 700; font-size: 14px; }
			.edb-empty-desc { font-size: 12px; color: var(--text-muted); }
			.edb-empty .btn { margin-top: 4px; margin-inline-end: 6px; }
		...
		<div class="edb-wrap">
			${!((data.cards || {}).active_projects) && frappe.user.has_role("System Manager") ? `
			<div class="edb-empty">
				<div class="edb-empty-title">${__("No projects yet — load the demo to explore the solution")}</div>
				<div class="edb-empty-desc">${__("Creates bilingual masters and 4 projects at different stages (Design, Tendering, Execution, Closed) with the complete record chain. Use a test site.")}</div>
				<button class="btn btn-primary btn-sm" onclick="cur_page.page.__load_demo()">${__("Load Demo Data")}</button>
				<button class="btn btn-default btn-sm" onclick="cur_page.page.__create_demo_user()">${__("Create Customer Test User")}</button>
			</div>` : ""}
...
function load_demo(page) {
	frappe.confirm(
		__("Load the full demo dataset (masters + 4 projects with the complete record chain)? Use a test site, not production."),
		() => {
			frappe.dom.freeze(__("Loading demo data — about 30 seconds…"));
			frappe.call("neo_engineering.api.demo.load_demo_data")
				.always(() => frappe.dom.unfreeze())
				.then(() => load(page));
		});
}

function delete_demo(page) {
	frappe.confirm(__("Remove all demo projects and their records?"), () => {
		frappe.dom.freeze(__("Removing demo data…"));
		frappe.call("neo_engineering.api.demo.delete_demo_data")
			.always(() => frappe.dom.unfreeze())
			.then(() => load(page));
	});
}
...
function create_demo_user() {
	frappe.call("neo_engineering.api.demo.create_demo_user").then(r => {
		const d = r.message || {};
		frappe.msgprint({
			title: __("Customer test user ready"),
			indicator: "green",
			message: `<b>${__("Email")}:</b> ${d.email}<br>
				<b>${__("Password")}:</b> <code>${frappe.utils.escape_html(d.password || "")}</code><br><br>
				${frappe.utils.escape_html(d.note || "")}`,
		});
	});
}
