...
def _quotation(customer, brief, service_type, amount, submit=True):
    ...
    try:
        doc = frappe.get_doc({
            ...
            "items": [{"item_code": DEMO_ITEM, "qty": 1, "rate": amount,
                       "item_name": "Engineering Services | خدمات هندسية",
                       "description": "Design & supervision services per brief | "
                                      "خدمات التصميم والإشراف حسب الموجز"}],
        })
    ...
    try:
      with _workflow_bypass():
        ...
        if frappe.db.exists("Item", "ENG-WORKS-DEMO"):
            _try_delete_master("Item", "ENG-WORKS-DEMO")
