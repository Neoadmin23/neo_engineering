...
@contextmanager
def _workflow_bypass():
    """Temporarily disable workflow transition validation while seeding.

    Demo records are inserted directly in their illustrative state
    ("Approved", "Awarded", …), which the workflow engine would reject as
    an invalid transition from Draft. frappe.flags.in_import is not honored
    for this on all builds, so we stub validate_workflow wherever Frappe
    binds it (module and document-module import sites), and restore it
    afterwards — even on error."""
    import frappe.model.workflow as _wfm
    patched = []
    modules = [_wfm]
    try:
        import frappe.model.document as _docm
        modules.append(_docm)
    except Exception:
        pass
    for m in modules:
        fn = getattr(m, "validate_workflow", None)
        if callable(fn):
            patched.append((m, fn))
            m.validate_workflow = lambda doc: None
    try:
        yield
    finally:
        for m, fn in patched:
            m.validate_workflow = fn
...
def make_demo_data():
    ...
    try:
        with _workflow_bypass():
            return _make_demo_data_inner()
    finally:
        frappe.flags.in_import = False


def _make_demo_data_inner():
    """Each project loads inside its own savepoint and commits on success,
    so one failure can never roll back the rest. Failures are logged with
    the project name and returned for the UI to display."""
    results = []
    company = _default_company()
    _masters()
    customers = [_customer(n) for n in CUSTOMERS]
    suppliers = [_supplier(*s) for s in SUPPLIERS]
    frappe.db.commit()  # masters persist regardless of project outcomes
    for idx, spec in enumerate(PROJECTS):
        ...
        try:
            ...
            frappe.db.commit()
            results.append({"project": spec[0], "status": "ok", "error": ""})
        except Exception as e:
            frappe.db.rollback(save_point=sp)
            frappe.log_error(frappe.get_traceback(),
                f"NEO-DEMO failed: {spec[0]}")
            results.append({"project": spec[0], "status": "failed",
                            "error": str(e)[:300]})
    sp = "neo_demo_pipeline"
    frappe.db.savepoint(sp)
    try:
        _pipeline_extras(customers)
        frappe.db.commit()
        results.append({"project": "Pipeline extras", "status": "ok", "error": ""})
    except Exception as e:
        frappe.db.rollback(save_point=sp)
        frappe.log_error(frappe.get_traceback(), "NEO-DEMO failed: pipeline")
        results.append({"project": "Pipeline extras", "status": "failed",
                        "error": str(e)[:300]})
    for r in results:
        mark = "✔" if r["status"] == "ok" else "✘"
        print(f"  {mark} {r['project']}" + (f" — {r['error']}" if r["error"] else ""))
    return results
...
# ------------------------------------------------------------------ CRM leg
def _lead(project_name, customer_display, location, budget, service_type, ptype):
    existing = frappe.db.get_value("Lead", {"company_name": customer_display,
        "custom_project_location": location}, "name")
    if existing:
        return existing
    doc = frappe.get_doc({
        "doctype": "Lead",
        "first_name": "Eng. Khalid | م. خالد",
        "company_name": customer_display,
        "company": _default_company(),
        "status": "Open",
        "custom_project_type": ptype,
        "custom_project_location": location,
        "custom_estimated_budget": budget,
        "custom_required_services": service_type,
    }).insert(ignore_permissions=True)
    return doc.name


def _opportunity(customer, brief, service_type, project_name):
    existing = frappe.db.get_value("Opportunity",
        {"party_name": customer, "custom_project_brief": brief}, "name")
    if existing:
        return existing
    doc = frappe.get_doc({
        "doctype": "Opportunity", "opportunity_from": "Customer",
        "party_name": customer, "status": "Converted",
        "company": _default_company(), "transaction_date": nowdate(),
        "custom_project_brief": brief, "custom_service_type": service_type,
        "title": project_name,
    }).insert(ignore_permissions=True)
    return doc.name


def _quotation(customer, brief, service_type, amount, submit=True):
    existing = frappe.db.get_value("Quotation",
        {"party_name": customer, "custom_project_brief": brief,
         "docstatus": ("<", 2)}, "name")
    if existing:
        return existing
    try:
        doc = frappe.get_doc({
            ...
            "party_name": customer, "company": _default_company(),
            "order_type": "Sales",
            "custom_project_brief": brief, "custom_service_type": service_type,
            "custom_proposal_version": "Rev 1",
            "items": [
                {"item_code": "ENG-DESIGN-DEMO", "qty": 1,
                 "rate": round(amount * 0.4),
                 "description": "Concept & detailed design per brief | "
                                "التصميم الابتدائي والتفصيلي حسب الموجز"},
                {"item_code": "ENG-SUPERVISION-DEMO", "qty": 1,
                 "rate": round(amount * 0.6),
                 "description": "Site supervision through handover | "
                                "الإشراف على الموقع حتى التسليم"},
            ],
        })
        doc.insert(ignore_permissions=True)
        if submit:
            doc.submit()
            doc.db_set("status", "Ordered", update_modified=False)
        return doc.name
    except Exception:
        frappe.log_error(frappe.get_traceback(), "NEO-DEMO quotation")
        print("  ! Quotation skipped (selling defaults missing) — see Error Log.")
        return None


def _pipeline_extras(customers):
    """Live funnel records the customer can watch move: an open lead and an
    opportunity at quotation stage with a draft proposal."""
    if not frappe.db.exists("Lead",
            {"company_name": "Riyadh Schools Extension | توسعة مدارس الرياض"}):
        frappe.get_doc({
            "doctype": "Lead", "first_name": "Mr. Fahad | أ. فهد",
            "company_name": "Riyadh Schools Extension | توسعة مدارس الرياض",
            "company": _default_company(),
            "status": "Open",
            "custom_project_type": PROJECT_TYPES[1],
            "custom_project_location": "Riyadh – Al Narjis | الرياض – النرجس",
            "custom_estimated_budget": 5_000_000,
            "custom_required_services": "Design + Supervision",
        }).insert(ignore_permissions=True)
    opp_title = "Warehouse Complex Study | دراسة مجمع مستودعات"
    if not frappe.db.exists("Opportunity", {"title": opp_title}):
        opp = frappe.get_doc({
            "doctype": "Opportunity", "opportunity_from": "Customer",
            "party_name": customers[1], "status": "Quotation",
            "company": _default_company(), "transaction_date": nowdate(),
            "custom_service_type": "Design Only", "title": opp_title,
        }).insert(ignore_permissions=True)
        _quotation(customers[1], None, "Design Only", 480000, submit=False)
...
def _project_with_records(idx, spec, customers, suppliers, company):
    ...
    lead = _lead(project_name, CUSTOMERS[cidx], location,
                 2_500_000 * (idx + 1), service_type, PROJECT_TYPES[tidx])
    brief = _brief(project_name, customer, location, lead=lead)
    opportunity = _opportunity(customer, brief, service_type, project_name)
    _quotation(customer, brief, service_type, 2_500_000 * (idx + 1))
    frappe.db.set_value("Lead", lead, {
        "custom_project_brief": brief, "status": "Converted",
    }, update_modified=False)
    frappe.db.set_value("Project Brief", brief, {
...
def _brief(project_name, customer, location, lead=None):
    ...
    if existing:
        return existing
    doc = frappe.get_doc({
        "doctype": "Project Brief", "customer": customer, "lead": lead,
...
def delete_demo_data():
    ...
    try:
      with _workflow_bypass():
        ...
        # Orphan briefs from partial loads (brief created, project failed)
        demo_cust_for_briefs = frappe.get_all("Customer",
            filters={"customer_name": ("in", CUSTOMERS)}, pluck="name")
        for b in frappe.get_all("Project Brief",
                filters={"customer": ("in", demo_cust_for_briefs or [""])},
                pluck="name"):
            doc = frappe.get_doc("Project Brief", b)
            if doc.docstatus == 1:
                doc.cancel()
            frappe.delete_doc("Project Brief", b, force=True,
                ignore_permissions=True)
        # CRM leg cleanup (matched by demo customers / demo names)
        demo_customers = frappe.get_all("Customer",
            filters={"customer_name": ("in", CUSTOMERS)}, pluck="name")
        for q in frappe.get_all("Quotation",
                filters={"party_name": ("in", demo_customers or [""])}, pluck="name"):
            doc = frappe.get_doc("Quotation", q)
            if doc.docstatus == 1:
                doc.cancel()
            frappe.delete_doc("Quotation", q, force=True, ignore_permissions=True)
        for o in frappe.get_all("Opportunity",
                filters={"party_name": ("in", demo_customers or [""])}, pluck="name"):
            frappe.delete_doc("Opportunity", o, force=True, ignore_permissions=True)
        lead_names = [c for c in CUSTOMERS] + [
            "Riyadh Schools Extension | توسعة مدارس الرياض"]
        for l in frappe.get_all("Lead",
                filters={"company_name": ("in", lead_names)}, pluck="name"):
            frappe.delete_doc("Lead", l, force=True, ignore_permissions=True)
        for code, _n, _s2, _p2 in DEMO_ITEMS:
            if frappe.db.exists("Item", code):
                _try_delete_master("Item", code)
        if frappe.db.exists("Item Group", DEMO_ITEM_GROUP):
            _try_delete_master("Item Group", DEMO_ITEM_GROUP)
        # Masters: delete only if nothing else links to them. No force —
        # Frappe's link validation is the "no other transactions" check,
        # so customers/suppliers used by real records are kept.
        for cust in frappe.get_all("Customer",
                filters={"customer_name": ("in", CUSTOMERS)}, pluck="name"):
            _try_delete_master("Customer", cust)
        for sup in frappe.get_all("Supplier",
                filters={"supplier_name": ("in", [x[0] for x in SUPPLIERS])},
                pluck="name"):
            _try_delete_master("Supplier", sup)
        for pt in PROJECT_TYPES:
            if frappe.db.exists("Project Type", pt):
                _try_delete_master("Project Type", pt)
        for uom in ["Lump Sum | مقطوعية", "Running Meter | متر طولي"]:
            if frappe.db.exists("UOM", uom):
                _try_delete_master("UOM", uom)
        frappe.db.commit()
...
# ------------------------------------------------------------ financial leg
DEMO_ITEM_GROUP = "Engineering Services | خدمات هندسية"
DEMO_ITEMS = [
    ("ENG-DESIGN-DEMO", "Design Services | خدمات التصميم", 1, 0),
    ("ENG-SUPERVISION-DEMO", "Supervision Services | خدمات الإشراف", 1, 0),
    ("ENG-EXEC-WORKS-DEMO", "Execution Works | أعمال التنفيذ", 0, 1),
]
DEMO_ITEM = "ENG-EXEC-WORKS-DEMO"  # purchase-side item for PO / PI


def _demo_items():
    if not frappe.db.exists("Item Group", DEMO_ITEM_GROUP):
        frappe.get_doc({
            "doctype": "Item Group", "item_group_name": DEMO_ITEM_GROUP,
            "parent_item_group": frappe.db.get_value(
                "Item Group", {"is_group": 1, "parent_item_group": ""}, "name")
                or "All Item Groups",
            "is_group": 0,
        }).insert(ignore_permissions=True)
    for code, name, is_sales, is_purchase in DEMO_ITEMS:
        if frappe.db.exists("Item", code):
            continue
        frappe.get_doc({
            "doctype": "Item", "item_code": code, "item_name": name,
            "description": name, "item_group": DEMO_ITEM_GROUP,
            "stock_uom": "Nos", "is_stock_item": 0,
            "is_sales_item": is_sales, "is_purchase_item": is_purchase,
        }).insert(ignore_permissions=True)


def _demo_item(company=None):  # kept for backward compatibility
    _demo_items()
    return DEMO_ITEM
...
def _try_delete_master(doctype, name):
    """Delete a demo master only when nothing else links to it."""
    try:
        frappe.delete_doc(doctype, name, ignore_permissions=True)
        print(f"  removed {doctype}: {name}")
    except frappe.LinkExistsError:
        print(f"  kept {doctype} {name} — other transactions exist")
    except Exception:
        frappe.log_error(frappe.get_traceback(),
            f"NEO-DEMO: could not delete {doctype} {name}")
        print(f"  kept {doctype} {name} — see Error Log")
