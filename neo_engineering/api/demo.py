...
def load_demo_data():
    ...
    results = make_demo_data() or []
    status = demo_status()
    failed = [r for r in results if r.get("status") == "failed"]
    lines = "".join(
        f"<div>{'✔' if r['status'] == 'ok' else '✘'} "
        f"{frappe.utils.escape_html(r['project'])}"
        + (f"<br><code style='font-size:11px'>"
           f"{frappe.utils.escape_html(r['error'])}</code>" if r.get("error") else "")
        + "</div>"
        for r in results)
    frappe.msgprint(lines or _("Nothing to load."),
        indicator="orange" if failed else "green",
        title=_("Demo load — {0} ok, {1} failed").format(
